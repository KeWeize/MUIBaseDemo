...
import vit.com.mui.alpha.MUIAlphaFrameLayout;

/**
 * @author kewz
 * @date 2019/5/4
 */
public class MUIFrameLayout extends MUIAlphaFrameLayout implements IMUILayout {

    private MUILayoutHelper mLayoutHelper;

    public MUIFrameLayout(Context context) {
        super(context);
        init(context, null, 0);
    }

    public MUIFrameLayout(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs, 0);
    }

    public MUIFrameLayout(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.MUIFrameLayout,
                defStyleAttr, 0);
        int count = ta.getIndexCount();
        for (int i = 0; i < count; ++i) {
            int index = ta.getIndex(i);
            if (index == R.styleable.MUIFrameLayout_android_maxWidth) {
                builder.mWidthLimit = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_android_maxHeight) {
                builder.mHeightLimit = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_android_minWidth) {
                builder.mWidthMini = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_android_minHeight) {
                builder.mHeightMini = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_topDividerColor) {
                builder.mTopDividerColor = ta.getColor(index,
                        ContextCompat.getColor(context, R.color.mui_config_color_separator));
            } else if (index == R.styleable.MUIFrameLayout_mui_topDividerHeight) {
                builder.mTopDividerHeight = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_topDividerInsetLeft) {
                builder.mTopDividerInsetLeft = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_topDividerInsetRight) {
                builder.mTopDividerInsetRight = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_bottomDividerColor) {
                builder.mBottomDividerColor = ta.getColor(index,
                        ContextCompat.getColor(context, R.color.mui_config_color_separator));
            } else if (index == R.styleable.MUIFrameLayout_mui_bottomDividerHeight) {
                builder.mBottomDividerHeight = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_bottomDividerInsetLeft) {
                builder.mBottomDividerInsetLeft = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_bottomDividerInsetRight) {
                builder.mBottomDividerInsetRight = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_leftDividerColor) {
                builder.mLeftDividerColor = ta.getColor(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_leftDividerWidth) {
                builder.mLeftDividerWidth = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_leftDividerInsetTop) {
                builder.mLeftDividerInsetTop = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_leftDividerInsetBottom) {
                builder.mLeftDividerInsetBottom = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_rightDividerColor) {
                builder.mRightDividerColor = ta.getColor(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_rightDividerWidth) {
                builder.mRightDividerWidth = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_rightDividerInsetTop) {
                builder.mRightDividerInsetTop = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_rightDividerInsetBottom) {
                builder.mRightDividerInsetBottom = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_borderColor) {
                builder.mBorderColor = ta.getColor(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_borderWidth) {
                builder.mBorderWidth = ta.getDimensionPixelSize(index, 1);
            } else if (index == R.styleable.MUIFrameLayout_mui_radius) {
                builder.radius = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_outerNormalColor) {
                builder.mOuterNormalColor = ta.getColor(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_hideRadiusSide) {
                builder.mHideRadiusSide = ta.getColor(index, HIDE_RADIUS_SIDE_NONE);
            } else if (index == R.styleable.MUIFrameLayout_mui_showBorderOnlyBeforeL) {
                builder.mIsShowBorderOnlyBeforeL = ta.getBoolean(index, true);
            } else if (index == R.styleable.MUIFrameLayout_mui_shadowElevation) {
                builder.shadow = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_shadowAlpha) {
                builder.mShadowAlpha = ta.getFloat(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_useThemeGeneralShadowElevation) {
                builder.useThemeGeneralShadowElevation = ta.getBoolean(index, false);
            } else if (index == R.styleable.MUIFrameLayout_mui_outlineInsetLeft) {
                builder.mOutlineInsetLeft = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_outlineInsetRight) {
                builder.mOutlineInsetRight = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_outlineInsetTop) {
                builder.mOutlineInsetTop = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_outlineInsetBottom) {
                builder.mOutlineInsetBottom = ta.getDimensionPixelSize(index, 0);
            } else if (index == R.styleable.MUIFrameLayout_mui_outlineExcludePadding) {
